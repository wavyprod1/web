    ...
    <section id="portfolio" class="audio-portfolio-section surface-section px-4 py-8 md:px-6 lg:px-8">
      <div class="section-content-container mx-auto">
        <div class="text-center mb-6">
          <h2 class="text-4xl font-bold mb-3">Ascolta il Mio <span class="text-primary">Lavoro</span></h2>
          <p class="text-xl text-color-secondary line-height-3">Una selezione di brani che mostrano diversi stili e competenze.</p>
        </div>
          ...
          <div class="flex flex-column md:flex-row md:align-items-center gap-4">
            ...
            <div class="flex-grow-1">
              ...
              <p class="mt-0 mb-1 text-color-secondary">
                <i class="pi pi-tag mr-2"></i>{{ selectedTrack.genre }} <span v-if="selectedTrack.label" class="ml-2">| <i class="pi pi-building mx-2"></i>{{ selectedTrack.label }}</span>
              </p>
              <div class="mt-2">
                <span class="font-semibold mr-2">Ruoli:</span>
                <Tag v-for="role in selectedTrack.roles" :key="role" :value="role" severity="contrast" class="mr-1 mb-1"></Tag>
              </div>
            </div>
        ...
        <!-- Messaggio se nessuna traccia è selezionata (non dovrebbe apparire con l'init) -->
         <div v-else class="text-center text-color-secondary p-5 surface-card border-round shadow-2 mb-5">
            Caricamento player...
         </div>
                ...
                <div class="grid grid-nogutter">
                  <div v-for="(item, index) in slotProps.items" :key="item.id" class="col-12">
                    ...
                    >
                      ...
                      <i v-if="selectedTrack && item.id === selectedTrack.id" class="pi pi-volume-up text-primary text-xl ml-3"></i>
